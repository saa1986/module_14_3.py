...
# Цена: <number * 100>' 4 раза. После каждой надписи выводите картинки к продуктам.
# В конце выведите ранее созданное Inline меню с надписью "Выберите продукт для покупки:".
@dp.message(F.text == "Купить")  # Обработчик сообщений, который срабатывает, когда пользователь отправляет текст купить
async def get_buying_list(message: Message):  # Асинхронная функция для обработки покупки
    for i, product in enumerate(data, start=1):  # Перебираем список продуктов с индексом, начиная с 1
        name = product["name"]  # Получаем название текущего продукта
        description = product["description"]  # Получаем описание текущего продукта
        await message.answer(f"Название: {name} | Описание: {description} | Цена: {i * 100}")  # Отправляем сообщение
        # с названием, описанием и ценой продукта (цена = индекс * 100)
        img = FSInputFile(f'../14_3/p{i}.png')  # Загружаем изображение продукта из файловой системы по пути
        await message.answer_photo(img)  # Отправляем изображение продукта пользователю
    await message.answer("Выберите продукт для покупки:", reply_markup=await products_btn())  # После вывода всех
    # продуктов, запрашиваем выбор продукта с помощью динамически созданной клавиатуры
